	...
	$.chungTimePicker = function(element, options) {
		...
		function init() {
			...
			switch(obj.options.viewType) {
				...
				case 1:
					...
					break;
				default:
					console.log(obj.errorPrefix + 'viewType视图类型不存在，只能为0或者1，请输入正确的视图类型值。');
					break;
			}
		...
		function createHoursWrap() {
			var table = $('<table class="table-condensed"></table>'),
				title = $('<thead><tr><th colspan="6" class="title">小时</th></tr></thead>'),
				hours = $('<tbody></tbody>'),
		...
		function createMinutesWrap() {
			...
			switch(obj.options.viewType) {
				case 0:
					title.find('tr').append('<th class="prev js-back-hours"><i class="icon-arrow-left"></i></th><th colspan="4" class="title">分钟</th>');
					break;
				case 1:
					title.find('tr').append('<th colspan="6" class="title">分钟</th>');
					break;
				default:
					console.log(obj.errorPrefix + 'viewType视图类型不存在，只能为0或者1，请输入正确的视图类型值。');
					break;
		...
		function createBottomOperate() {
			switch(obj.options.viewType) {
				case 0:
					obj.timepicker_bottom.empty().append('<span class="bottom-btn js-clear">清除</span>')
						.append('<span class="bottom-btn js-cancel">取消</span>');
					break;
				case 1:
					obj.timepicker_bottom.empty().append('<span class="bottom-btn js-clear">清除</span>')
						.append('<span class="bottom-btn js-cancel">取消</span>')
						.append('<span class="bottom-btn js-confirm">确定</span>');
